...
+ (instancetype)keyBoardWithNavgationBarTranslucent:(BOOL)translucent
{
    ...
    if (translucent) {
        frame = CGRectMake(leftX, kScreenHeight - kChatToolBarHeight, kScreenWidth, kChatKeyBoardHeight);
    }else {
        frame = CGRectMake(leftX, kScreenHeight - kChatToolBarHeight - 64, kScreenWidth, kChatKeyBoardHeight);
    }
...
- (void)chatToolBar:(ChatToolBar *)toolBar moreBtnPressed:(BOOL)select keyBoardState:(BOOL)change
{
    if (select && change == NO)
    {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            self.lastChatKeyboardY = self.frame.origin.y;
            self.frame = CGRectMake(leftX, [self getSuperViewH]-CGRectGetHeight(self.frame), kScreenWidth, CGRectGetHeight(self.frame));
            self.morePanel.frame = CGRectMake(leftX, CGRectGetHeight(self.frame)-kMorePanelHeight, CGRectGetWidth(self.frame), kMorePanelHeight);
            self.facePanel.frame = CGRectMake(leftX, CGRectGetHeight(self.frame), CGRectGetWidth(self.frame), kFacePanelHeight);
...
{
    if (!_chatToolBar) {
        _chatToolBar = [[ChatToolBar alloc] initWithFrame:CGRectMake(leftX, 0, kScreenWidth, kChatToolBarHeight)];
...
{
    if (!_facePanel) {
        _facePanel = [[FacePanel alloc] initWithFrame:CGRectMake(leftX, kChatKeyBoardHeight-kFacePanelHeight, kScreenWidth, kFacePanelHeight)];
...
{
    if (!_OAtoolbar) {
        _OAtoolbar = [[OfficialAccountToolbar alloc] initWithFrame:CGRectMake(leftX, CGRectGetHeight(self.frame), kScreenWidth, kChatToolBarHeight)];
    }
    return _OAtoolbar;
}
